...
func TestHitsFromItemsMapsGitHubDomains(t *testing.T) {
	items := []Item{{
	...
	hits := HitsFromItems(DomainCode, items)
	if len(hits) != 1 {
...
func TestHitsFromItemsMapsIssueRepositoryURL(t *testing.T) {
	hits := HitsFromItems(DomainIssue, []Item{{
		Number:        12,
		Title:         "Fix parser",
		State:         "open",
...
func TestHitsFromItemsMapsCommitAndRepo(t *testing.T) {
	commitHits := HitsFromItems(DomainCommit, []Item{{
		SHA:        "abcdef123456",
	...
	repoHits := HitsFromItems(DomainRepo, []Item{{
