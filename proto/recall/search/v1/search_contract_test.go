...
func TestSearchRequestV1KeepsStableQueryLimitAndKindHints(t *testing.T) {
	fields := File_proto_recall_search_v1_search_proto.Messages().ByName("SearchRequest").Fields()
	if fields.Len() != 3 {
		t.Fatalf("SearchRequest field count = %d, want query, limit, and kind_hints", fields.Len())
	}
	...
	if field := fields.ByNumber(3); field == nil || string(field.Name()) != "kind_hints" || !field.IsList() {
		t.Fatalf("field 3 = %v, want repeated kind_hints", field)
	}
}
